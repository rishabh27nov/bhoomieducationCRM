...
export function firebaseUrl(path, req) {
  const token = String(req?.headers?.authorization || '').match(/^Bearer\s+(.+)$/i)?.[1];
  const url = `${FIREBASE_URL}/${path}.json`;
  return token ? `${url}?auth=${encodeURIComponent(token)}` : url;
}
export async function readData(node, req) {
  const response = await fetch(firebaseUrl(node, req));
...
export async function authorize(req, res, { phone, adminOnly = false } = {}) {
  ...
  const employees = values(await readData('employees', req));
  ...
  if (header.startsWith('Basic ')) {
    ...
    // Google accounts normally have a verified email. Email/password accounts
    // can legitimately be unverified, so accept either provider once Firebase
    // has validated its ID token. Disabled Firebase accounts remain blocked.
    const hasPasswordProvider = identity?.providerUserInfo?.some(provider => provider.providerId === 'password');
    if (response.ok && !identity?.disabled && (identity?.emailVerified || hasPasswordProvider)) {
  ...
  const leads = values(await readData('leads', req));

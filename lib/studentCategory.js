...
export function importClassification(employee, fallbackCategory, salesSegment) {
  const category = employee?.category || fallbackCategory;
  return category === 'Academic'
    ? { studentCategory: 'Academic', leadType: 'Academic', stage: ACADEMIC_STAGES[0] }
    : { studentCategory: 'Sales', leadType: salesSegment === 'B2B2C' ? 'B2B2C' : 'B2C' };
}
